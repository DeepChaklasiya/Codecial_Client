    ...
    const fetchUser = async () => {
      try {
        const res = await axios.get(
          `https://codecial-server.herokuapp.com/api/users?username=${username}`
        );
        setUser(res.data);
      } catch (err) {
      ...
      try {
        ...
        if (fileProfile) {
          ...
          const res = await axios.put(
            `https://codecial-server.herokuapp.com/api/users/${user._id}`,
            updatedUser
          );
    ...
    if (changeUsername.length < 3 || changeUsername.length > 20) {
      ...
      try {
        const fetchedUser = await axios.get(
          `https://codecial-server.herokuapp.com/api/users/editProfile?username=${changeUsername}`
        );
        if (
          ...
          const updatedUser = await axios.put(
            `https://codecial-server.herokuapp.com/api/users/${currentUser._id}`,
            newUser
          );
          localStorage.removeItem('user');
          localStorage.clear();
